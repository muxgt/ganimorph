...
class VisualizeTestSet(Callback):
    ...
    def _trigger(self):
        ...
        for iA, iB in self.val_ds.get_data():
            ...
            cv2.imwrite('./output/realA-{}'.format(idx) + '.jpg', iA)
            cv2.imwrite('./output/realB-{}'.format(idx) + '.jpg', iB)
            cv2.imwrite('./output/fakeA-{}'.format(idx) + '.jpg', vizB)
            cv2.imwrite('./output/fakeB-{}'.format(idx) + '.jpg', vizA)
            idx += 1
